...
{
    int             connectivity, num_nodes;/* Connectivity and number of nodes in the tree */
    short int       *pr, *pi;               /* Pointers to image data */
    mwSize          m, n, aval;             /* Image size */
    int             bval, mi;
    ...
    mxArray         *fout;
    mwSize          i, j ,k, tmp1, tmp2, tmp3, *tmp0;
    int             *tmpi, ID;
    ...
    ima->version=2; ima->x=(sizs) m; ima->y=(sizs)n; ima->grid='s';
    ...
    for (i=0; i<GREY_LEVELS; i++){
        if (tree->level[i].nnodes!=0) {
            for (n=0; n<tree->level[i].nnodes; n++) {
                ...
                // mexPrintf("Level i: %d, node n: %d, nodeID k: %d \n",i,n,k);
                k++;
            }
        }
    }
    ...
    // mexPrintf("Final number of nodes: %d  \n",num_nodes);
    ...
    aval = 1;
    bval = 6;
    plhs[0] = mxCreateStructMatrix(num_nodes, aval,bval,(const char **)fnames);
    ...
    for (i=0; i<GREY_LEVELS; i++){
        if (tree->level[i].nnodes!=0) {
            for (n=0; n<tree->level[i].nnodes; n++) {
                /* Write the nodeID */ 
                ID   = tree->level[i].node[n].nodeID;
                fout = mxCreateNumericMatrix( aval, aval, mxINT32_CLASS, mxREAL);
                // tmp0 = (mwSize *) mxGetPr(fout); tmp0[0] = ID;
                tmpi = (int *) mxGetPr(fout); tmpi[0] = (int) ID;
                bval = 0;
                mxSetFieldByNumber(plhs[0], (mwIndex)(ID-1), bval, fout);

                /* Write the Gray level */ 
                // fout = mxCreateDoubleScalar((double)i);
                fout = mxCreateNumericMatrix(aval,aval, mxINT16_CLASS, mxREAL);
                // tmp0 = (mwSize *) mxGetPr(fout); tmp0[0] = i;
                tmpi = (int *) mxGetPr(fout); tmpi[0] = (int) i;
                bval = 1;
                mxSetFieldByNumber(plhs[0], (mwIndex)(ID-1), bval, fout);

                /* Write the Parent ID */
                // mexPrintf("GreyLevel: %d, Node: %d, Parent (level,node): %d, %d  \n",i, n, tree->level[i].node[n].father.level,tree->level[i].node[n].father.node);
                tmp1 = (mwSize)(tree->level[i].node[n].father.level);
                tmp2 = (mwSize)(tree->level[i].node[n].father.node);
                fout = mxCreateNumericMatrix(aval,aval, mxINT32_CLASS, mxREAL);
                if (tmp1!=-1){
                    tmp3 = (mwSize)(tree->level[tmp1].node[tmp2].nodeID);
                    // tmp0 = (mwSize *) mxGetPr(fout); tmp0[0] = tmp3;
                    tmpi = (int *) mxGetPr(fout); tmpi[0] = (int) tmp3;
                    bval = 2;
                    mxSetFieldByNumber(plhs[0], (mwIndex)(ID-1), bval, fout);
                }

                /* Write the Children IDs */
                tmp1    = (mwSize)(tree->level[i].node[n].NChildren);   if (tmp1<1) {tmp1=1;}
                // mexPrintf("             Number of children: %d, tmp1: %d \n",tree->level[i].node[n].NChildren, tmp1);
                fout    = mxCreateNumericMatrix(aval, tmp1, mxINT32_CLASS, mxREAL);
                // tmp0    = (mwSize *) mxGetPr(fout);                  
                tmpi    = (int *) mxGetPr(fout);                  
                if (tree->level[i].node[n].NChildren>0){
                    for (mi=0; mi<tree->level[i].node[n].NChildren; mi++){
                        // mexPrintf("             Children: %d, (level,node): %d, %d \n",m,tree->level[i].node[n].Child[mi].level, tree->level[i].node[n].Child[mi].node);
                        tmp1 = (mwSize)(tree->level[i].node[n].Child[mi].level);
                        tmp2 = (mwSize)(tree->level[i].node[n].Child[mi].node);
                        tmp3 = (mwSize)(tree->level[tmp1].node[tmp2].nodeID);
                        // mexPrintf("tmp1: %d, tmp2: %d, tmp3: %d \n",tmp1, tmp2, tmp3);
                        // tmp0[mi] = 1;
                        tmpi[mi] = (int) tmp3;
                    }
                    bval = 3;
                ...
                tmpi = (int *) mxGetPr(fout); tmpi[0] = (int) tmp1;
                bval = 4;
                mxSetFieldByNumber(plhs[0], (mwIndex)(ID-1), bval, fout);
                ...
                // tmp0    = (mwSize *) mxGetPr(fout);                  
                tmpi    = (int *) mxGetPr(fout);                  
                for (mi=0; mi<tree->level[i].node[n].NPixels; mi++){
                    tmp3 = (mwSize)(tree->level[i].node[n].Pixel[mi]);
                    // tmp0[m] = tmp3;
                    tmpi[mi] =  (int) tmp3;
                }
                bval = 5;
